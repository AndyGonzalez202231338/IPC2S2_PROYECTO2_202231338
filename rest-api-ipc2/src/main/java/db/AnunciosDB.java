...
public class AnunciosDB {
    ...
    private static final String SELECT_ANUNCIOS_CON_PUBLICIDAD_ACTIVA
            = "SELECT a.*, "
            + "ta.id_tipo_anuncio, ta.nombre AS tipo_nombre, ta.descripcion AS tipo_descripcion, ta.precio_anuncio, "
            + "pa.id_periodo, pa.nombre AS periodo_nombre, pa.dias_duracion, pa.precio_duracion, "
            + "p.id_publicidad, p.precio_bloqueo AS precio_bloqueo_publicidad, p.estado AS estado_publicidad "
            + "FROM anuncio a "
            + "INNER JOIN publicidad p ON a.id_anuncio = p.id_anuncio "
            + "INNER JOIN tipos_anuncio ta ON a.id_tipo_anuncio = ta.id_tipo_anuncio "
            + "INNER JOIN periodos_anuncio pa ON a.id_periodo = pa.id_periodo "
            + "WHERE p.estado = 'ACTIVO' AND a.estado = 'ACTIVO'";
    ...
    /**
     * Método auxiliar para mapear un ResultSet a un objeto Anuncio completo
     * (con joins)
     */
    private Anuncio mapResultSetToAnuncioCompleto(ResultSet resultSet) throws SQLException {
        ...
        try {
            ...
            periodoAnuncio.setIdPeriodo(resultSet.getInt("id_periodo")); // Cambiado
    ...
    public List<Anuncio> getAnunciosConPublicidadActiva() {
        ...
        try (PreparedStatement stmt = conn.prepareStatement(SELECT_ANUNCIOS_CON_PUBLICIDAD_ACTIVA)) {
            ResultSet rs = stmt.executeQuery();

            while (rs.next()) {
                Anuncio anuncio = mapToAnuncioCompletoConPublicidad(rs);
            ...
            e.printStackTrace();
            throw new RuntimeException("Error al obtener anuncios con publicidad activa", e);
        }

        return anuncios;
    }

    private Anuncio mapToAnuncioCompletoConPublicidad(ResultSet rs) throws SQLException {
    Anuncio anuncio = mapResultSetToAnuncioCompleto(rs);
    
    // Agregar información de publicidad
    int idPublicidad = rs.getInt("id_publicidad");
    if (!rs.wasNull()) {
        Publicidad publicidad = new Publicidad();
        publicidad.setIdPublicidad(idPublicidad);
        publicidad.setIdAnuncio(rs.getInt("id_anuncio"));
        publicidad.setIdUsuario(rs.getInt("id_usuario"));
        publicidad.setPrecioBloqueo(rs.getBigDecimal("precio_bloqueo_publicidad"));
        publicidad.setEstado(rs.getString("estado_publicidad"));
        anuncio.setPublicidad(publicidad); // ¡FALTABA ESTA LÍNEA!
        
        // DEBUG para publicidad
        System.out.println("=== DEBUG PUBLICIDAD ===");
        System.out.println("Publicidad ID: " + publicidad.getIdPublicidad());
        System.out.println("Anuncio ID: " + publicidad.getIdAnuncio());
        System.out.println("Precio Bloqueo: " + publicidad.getPrecioBloqueo());
        System.out.println("Estado Publicidad: " + publicidad.getEstado());
    } else {
        System.out.println("=== DEBUG PUBLICIDAD ===");
        System.out.println("No se encontró publicidad para el anuncio ID: " + anuncio.getIdAnuncio());
    }
